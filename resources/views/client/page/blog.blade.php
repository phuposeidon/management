        ...
        <div class="row">
          ...
          <div class="banner-info">
            ...
            <div class="banner-text text-center">
              <h1 class="white">Hỏi đáp cùng bác sĩ
              @if (session('questionId'))
                  <div hidden id="questionId">
                      {{ session('questionId') }}
                  </div>
              @endif
              
              </h1>
              <p>Nơi bạn đặt ra những câu hỏi, vấn đề bạn và người thân mắc phải
                <br>Đội ngũ y sĩ của chúng tôi sẽ sẵn sàng trả lời.</p>
            ...
            <div class="service-info blog-border">
              <div class="row blog-content">
                ...
                <div class="col-md-11">
                  ...
                  <div class="question">
                    <textarea id="content" name="content" rows="10" placeholder="Bạn muốn hỏi bác sĩ điều gì?"></textarea>
                  </div>

                  <div class="question-pic" id="gallery"></div>

                  <button type="button" class="btn btn-danger" id="del-btn">
              ...
              <div class="row blog-footer pad-10">
                ...
                <div class="col-md-2">
                  <input id="anonymous" type="checkbox" name="anonymous"> Đăng ẩn danh
                </div>
                <div class="col-md-1">
          ...
          <div class="service-info blog-border" id="post-{{$question->id}}">
            <div class="row blog-content">
              <div class="user-info row">
                <div class="col-md-1 avatar-user">
                  <img src="@if($question->anonymous == 0){{asset('img/patient/'.$question->Patient->avatar)}} @else {{asset('img/patient/user-default.png')}} @endif"
                    alt="">
                </div>
                <div class="col-md-10 about-user">
                  <p>
                    <span class="name">
                      @if($question->anonymous == 1) Thành viên giấu tên @else {{$question->Patient->fullname}} @endif
                    </span>
                    <span> đã hỏi</span>
                  </p>
                  <small>
                    <span>
                      @if(Carbon\Carbon::today()->format('Y-m-d') == Carbon\Carbon::Parse($question->createdAt)->format('Y-m-d')) Hôm nay, {{Carbon\Carbon::Parse($question->createdAt)->format('H:i')}}
                    ...
                    <span>@if($question->Patient->gender == 1) Nam @else Nữ @endif
                    </span>
                    <span>
              ...
              <div class="question-pic">
                <ul class="list-pic">
                  <?php $questionImgs = App\QuestionImage::where('questionId', $question->id)->get();?> @foreach($questionImgs as $img)
                  <li class="picture">
                    <img src="{{asset('img/questionimage/'.$question->id.'/'.$img->url)}}">
                  </li>
                  @endforeach
                </ul>
              </div>
            ...
            <div class="blog-footer">
              ...
              <div class="count-like pad-10">
                <i class="fa fa-thumbs-o-up fa-fw"></i>
                <a id="question-like-{{$question->id}}">{{count(App\Like::where('questionId', $question->id)->get())}}</a>
                <i class="fa fa-comment-o fa-fw"></i>
                <a id="question-reply-{{$question->id}}"> {{count($answers)}}</a>
              </div>

              <div class="more-detail pad-10">
                <a> Xem tất cả {{count($answers)}} trả lời... </a>
              </div>
              ...
              <div id="question-{{$question->id}}" style="margin-top: 10px; ">
                ...
                <div class="user-answer blog-content row">
                  <div class="avatar-user col-md-1">
                    <img src="@if(isset($answer->Patient->fullname)) {{asset('img/patient/'.$answer->Patient->avatar)}} @else {{asset('img/user/'.$answer->User->avatar)}} @endif"
                      alt="">
                  </div>
                  <div class="col-md-11 pad-10">
                    ...
                    @else
                    <b>
                      <i class="fa fa-user-md" style="color: #5cb85c; font-size: 25px"></i> Bác sĩ {{$answer->User->fullname}}</b>
                    @endif
              ...
              <div class="answerForm" id="af-{{$question->id}}">
                ...
                <div class="post-answer blog-content row">
                  ...
                  <div class="col-md-1" style="padding: 8px">
                    <center>
                      <button type="submit" id="f-{{$question->id}}" class="btn btn-success sendAnswer">Gửi</button>
                    </center>
                  </div>
                </div>
              </div>
  ...
  <script>
    $(document).ready(function () {
        ...
        if (input.files) {
          ...
          for (i = 0; i < filesAmount; i++) {
            ...
            reader.onload = function (event) {
              $($.parseHTML('<img>')).attr('src', event.target.result).addClass('input-image').appendTo(
      ...
      $('.btn-send').click(function () {
        ...
        var anonymous = $('#anonymous:checked').val();
        var avatar =  "{{asset('img/patient/'.Auth::guard('patient')->user()->avatar)}}";  
        var patient = "{{Auth::guard('patient')->user()->fullname}}";
        var infomation = {
          content:content,
          anonymous:anonymous,
          avatar:avatar,
        ...
        socket.emit('notification',infomation);
      });
      ...
      $('[id*="post-"]').each(function () {
        ...
        if ($(this).find('.blog-footer [id*="question-"] .user-answer').length > 2) {
          $(this).find('.blog-footer [id*="question-"] .user-answer').slice(0, ($(this).find(
            '.blog-footer [id*="question-"] .user-answer').length - 2)).hide();
          md.show();
      ...
      socket.on('answerSuccess', function (answer) {
       if(answer.doctor=='')
       {
          ...
          `
                <div class="user-answer blog-content row">
                  <div class="avatar-user col-md-1"><img src="img/patient/`+answer.patient_avatar+`" alt=""></div>
                  <div class="col-md-11 pad-10">
                    <b>` +
          answer.patient + `</b>
